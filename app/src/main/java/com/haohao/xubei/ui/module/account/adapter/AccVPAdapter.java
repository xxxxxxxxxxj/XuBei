package com.haohao.xubei.ui.module.account.adapter;

import androidx.annotation.NonNull;
import androidx.fragment.app.FragmentManager;
import androidx.fragment.app.FragmentPagerAdapter;
import android.view.ViewGroup;

import com.haohao.xubei.ui.module.account.AccRListFragment;

import java.util.List;

import javax.inject.Inject;

/**
 * 订单 ViewPage Adapter
 * date：2017/11/14 14:25
 * author：Seraph
 * mail：417753393@qq.com
 **/
public class AccVPAdapter extends FragmentPagerAdapter {

    private String[] orderType;
    private List<AccRListFragment> fragments;

    @Inject
    AccVPAdapter(FragmentManager fm, String[] orderType, List<AccRListFragment> fragments) {
        super(fm);
        this.orderType = orderType;
        this.fragments = fragments;
    }

    @Override
    public int getCount() {
        return orderType.length;
    }

    @Override
    public AccRListFragment getItem(int position) {
        return fragments.get(position);
    }


    @Override
    public CharSequence getPageTitle(int position) {
        return orderType[position];
    }

    @Override
    public void destroyItem(@NonNull ViewGroup container, int position,@NonNull Object object) {
    }
}
