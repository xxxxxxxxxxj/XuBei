...
import com.xubei.shop.databinding.ActMainMeBuyBinding;
import com.xubei.shop.ui.module.base.ABaseFragment;
import com.xubei.shop.ui.module.main.contract.MainMeBuySellContract;
import com.xubei.shop.ui.module.main.presenter.MainMeBuySellPresenter;
...
public class MainMeBuy extends ABaseFragment<MainMeBuySellContract.Presenter> implements MainMeBuySellContract.View {
    ...
    protected View initDataBinding(LayoutInflater inflater, ViewGroup container) {
        binding = DataBindingUtil.inflate(inflater, R.layout.act_main_me_buy, container, false);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.tv_order_buy1://待付款
                ARouter.getInstance().build(AppConstants.PagePath.ORDER_ALL).withInt("type", 2).navigation();
                break;
            case R.id.tv_order_buy2://租赁中
                ARouter.getInstance().build(AppConstants.PagePath.ORDER_ALL).withInt("type", 1).navigation();
                break;
            case R.id.tv_order_buy3://已完成
                ARouter.getInstance().build(AppConstants.PagePath.ORDER_ALL).withInt("type", 3).navigation();
                break;
            case R.id.tv_order_buy4://售后维权
                ARouter.getInstance().build(AppConstants.PagePath.ORDER_ALL).withInt("type", 4).navigation();
                break;
            default:
