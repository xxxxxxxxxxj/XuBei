...
import com.xubei.shop.databinding.ActAccDetailFreeBinding;
import com.xubei.shop.ui.module.account.adapter.AccDetailBannerAdapter;
...
import com.xubei.shop.ui.module.account.presenter.AccDetailPresenter;
 ...
 * author：Seraph
 **/
@Route(path = AppConstants.PagePath.ACC_DETAIL_FREE)
public class AccFreeDetailActivity extends ABaseActivity<AccDetailContract.Presenter> implements AccDetailContract.View {
    ...
    protected void initContextView() {
        binding = DataBindingUtil.setContentView(this, R.layout.act_acc_detail_free);
    ...
    public void setAccountDetail(OutGoodsDetailBean outGoodsDetailBean) {
        ...
        //商品状态
        binding.tvRent.setText("立即租用".equals(outGoodsDetailBean.getGoodsStatusStr()) ? "免费玩" : outGoodsDetailBean.getGoodsStatusStr());
        //判断是否是签约卖家或者热门推荐
    ...
    public void showPayInputPop(String orderNo, String timeLong, String kyGold, String payGoldNo) {
        dialogUtils.inputGoldSixPasswordDialog(binding.llRoot, orderNo, timeLong, kyGold, payGoldNo, new AlertDialogUtils.OnSixPasswordListener() {

            @Override
            public void onInputPassWord(String pass) {
                if (pass.length() == 6) {
                    dialogUtils.closeGoldInputPayPop();
                    presenter.startGoldPay(pass);
                }
            }
    ...
    public void showShareDialog() {
        dialogUtils.createShareDialog(binding.llRoot, str -> presenter.doShowFreeShare(str));
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.tv_rent://免费租赁
                presenter.doClickRentFree();
                break;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.menu_swxz, menu);
