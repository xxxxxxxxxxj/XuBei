package com.xubei.shop.ui.module.common.photolist;
...
public class LocalImageListPresenter extends LocalImageListContract.Presenter {
    ...
    private void startAsyncQuery() {
        mQueryHandler = new AsyncImageQueryHandler(mView);
        //检查读取存储卡的权限
    ...
    private static class AsyncImageQueryHandler extends AsyncQueryHandler {
            ...
            switch (token) {
                case CODE_REQUEST:
                    if (weakReference.get() != null) {
                        weakReference.get().setQueryImageList(initImageList(cursor));
                    }
                    break;
            }
        }
     ...
     * 获取数据
     */
    private static List<LocalImageBean> initImageList(Cursor cursor) {
